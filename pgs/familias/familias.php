...
<html>
  ...
  <body>
    ...
    <div class="container">
      <form action="ingresar_familia.php" method="post" class="needs-validation" novalidate>
        ...
        <div class="row" style="margin-bottom: 7px;">
          ...
          <div class="col-lg-10 col-md-9 col-sm-12">
            <input type="text" name="txtSrchFam" id="txtSrchFam" class="form-control" />
          </div>
        </div>
        <div class="table-responsive">
          <div class="table-wrapper-scroll-y my-custom-scrollbar">
            <table class="table table-bordered table-sm" id="tblFam">
              <thead class="thead-light">
                <tr id="trFam">
                  <th width="30%">Clase</th>
                  <th width="30%">Orden</th>
                  <th width="40%">Familia</th>
                </tr>
              </thead>
              <tbody>
              <?php
              $result = mysqli_query($conn, $_POST['qryFamilia']);
              mysqli_close($conn);
              ...
              ?>
                <tr>
                  <td width="30%"><?php echo $row["Clase"]; ?></td>
                  <td width="30%"><?php echo $row["Orden"]; ?></td>
                  <td width="40%"><a href="#Ingresar_Familia" onclick="FamilyClicked(this);"><?php echo $row["Familia"]; ?></a></td>
                </tr>
              <?php
              }
              ?>
        ...
        <div class="form-row" id="Ingresar_Familia">
          <div class="form-group col-lg-4 col-md-4 col-sm-12">
            <label for="">Familia escogida</label>
            <input type="text" class="form-control" name="txtFamilia" id="txtFamilia" readonly required value="">
          </div>
          <div class="form-group col-lg-4 col-md-4 col-sm-12">
            <label for="">Número</label>
            <input type="number" class="form-control" name="txtNumero" id="txtNumero" required value="">
          </div>
          <div class="form-group col-lg-4 col-md-4 col-sm-12">
    ...
    <form class="" action="../setdate.php" method="post" hidden>
      <input type="text" name="page" id="txtPage" value="familias/familias.php">
...
<script>
  ...
  //Fam Table
  $(document).ready(function(){
    $('#txtSrchFam').keyup(function(){
      search_table($(this).val());
    });
    function search_table(value){
      $('#tblFam tr').each(function(){
        var found = 'false';
        $(this).each(function(){
          if($(this).text().toLowerCase().indexOf(value.toLowerCase()) >= 0)
            found = 'true';
  ...
  function FamilyClicked(a)
  {
    document.getElementById('txtFamilia').value = a.innerHTML;
    $('#tblFam tr').each(function(){
      $(this).show();
    });

    document.getElementById('txtSrchFam').value = "";

    // Enabling main button
    if (document.getElementById("txtSitio").value != "" && document.getElementById('txtFamilia').value != "")
